...
def process_exits(state: Dict[str, Any],
    ...
    """
    Exit logic synced with backtest:
    - Fill uses NEXT candle's open (index + 1) instead of current close.
    - Added trailing stop tier at 1.5R (from backtest).
    """
    for symbol in list(state["positions"].keys()):
        df = data[symbol]

        # Need a next candle to fill — skip if at the last bar
        if index + 1 >= len(df):
            logger.warning("[EXIT SKIP] %s: no next candle available at index %d", symbol, index)
            continue
        ...
        # ── Trailing stop tier at 1.5R (added from backtest) ─────────────────
        if current_r >= 1.5:
            trailing = max(
                float(position["stop"]),
        ...
        if current_r >= 1.0 and not position.get("partial_taken", False):
            ...
            exit_price = float(next_row["open"]) * (1 - SLIPPAGE)   # next open
        ...
        # ── Full exit — fill on next candle's open ────────────────────────────
        exit_price = float(next_row["open"]) * (1 - SLIPPAGE)       # next open
...
def process_entries(state: Dict[str, Any], data: Dict[str, pd.DataFrame],
    ...
    """
    Entry logic synced with backtest:
    - BTC trend check: close > ema_4h  (was: ema_4h rising over 10 bars).
    - Fill uses NEXT candle's open (index + 1) instead of current close.
    """
    ...
    # BTC trend: close > ema_4h (synced with backtest; was ema_4h[i-10] rising)
    ...
    for symbol in ASSETS:
        ...
        # Fill on next candle's open (synced with backtest)
        entry_price = float(next_row["open"]) * (1 + SLIPPAGE)
        stop_price = entry_price - stop_distance
        size = trade_risk / stop_distance

        entry_type = "BREAKOUT" if breakout_entry else "PULLBACK"
        print(f"[ENTRY TYPE] {entry_type} {symbol} index={index}")
...
def process_pyramiding(state: Dict[str, Any], data: Dict[str, pd.DataFrame],
                       index: int, equity_ma: float) -> None:
    """
    Pyramiding logic synced with backtest:
    - Fill uses NEXT candle's open (index + 1) instead of current close.
    """
    total_open_risk = position_open_risk(state)
    equity = float(state["equity"])
    ...
    for symbol in list(state["positions"].keys()):
        ...
        # Fill on next candle's open (synced with backtest)
        add_price = float(next_row["open"]) * (1 + SLIPPAGE)
...
def main() -> None:
    ...
    # Reserve one bar for next-open fills — don't process the very last candle
    # since we need index+1 to exist for realistic fill execution.
    safe_latest_idx = latest_idx - 1

    candles_to_process = list(range(start_index, safe_latest_idx + 1))

    if len(candles_to_process) > 1:
        logger.warning(
            "CATCHUP: processing %d candles from index %d to %d.",
            len(candles_to_process), start_index, safe_latest_idx,
        )
    ...
    print_report(state, data, safe_latest_idx)
